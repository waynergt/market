...
import { EditProductModal } from './EditProductModal'; // Importamos el editor
    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Gestiona tus productos y existencias</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white p-4 rounded-3xl shadow-sm border border-gray-100">
        ...
        <div className="relative">
          ...
          <select 
            className="w-full pl-12 pr-4 py-3 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none appearance-none cursor-pointer"
        ...
        <div className="flex bg-gray-50 p-1 rounded-2xl">
          <button 
            ...
            className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${stockFilter === 'all' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-400'}`}
          >
            Todos
          </button>
          <button 
            onClick={() => setStockFilter('low')}
            className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1 ${stockFilter === 'low' ? 'bg-white shadow-sm text-orange-600' : 'text-gray-400'}`}
          >
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredProducts.map((product) => (
                <tr key={product.id} className="group hover:bg-blue-50/30 transition-colors">
                  <td className="p-5">
                    <div className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors">{product.name}</div>
                  ...
                  <td className="p-5">
                    <div className="flex items-center justify-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${product.stock < 5 ? 'bg-orange-500 animate-pulse' : 'bg-green-500'}`}></div>
                      <span className={`font-bold ${product.stock < 5 ? 'text-orange-600' : 'text-gray-700'}`}>
                        {product.stock}
                      </span>
                    </div>
                  </td>
                  <td className="p-5 text-right">
                    <button 
                      ...
                      className="text-gray-300 hover:text-blue-600 transition-colors p-2 hover:bg-blue-50 rounded-lg"
                    >
