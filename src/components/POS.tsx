...
export const POS = () => {
  ...
  const [manualBarcode, setManualBarcode] = useState(''); // Estado para ingreso manual
  const [isCustomerModalOpen, setIsCustomerModalOpen] = useState(false);

  const [lastSale, setLastSale] = useState<any>(null);
  const ticketRef = useRef<HTMLDivElement>(null);

  // --- LÓGICA PARA LECTOR FÍSICO ---
  useEffect(() => {
    let barcodeBuffer = '';
  ...
  const handleScan = async (code: string) => {
    ...
    if (data.stock <= 0) return alert("¡Producto agotado!");

    addItemToCart(data);
    setManualBarcode(''); // Limpiamos el campo manual si se usó
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-gray-50">
            ...
            <tbody className="divide-y divide-gray-50">
              {cart.map((item) => (
                <tr key={item.id} className="hover:bg-green-50/30 transition-colors group text-sm">
                  ...
                  <td className="p-6 text-center font-black">{item.quantity}</td>
                  <td className="p-6 text-right font-bold text-gray-500">Q{item.price.toFixed(2)}</td>
                  <td className="p-6 text-right font-black text-green-700 text-lg">Q{(item.price * item.quantity).toFixed(2)}</td>
      ...
      <div className="w-full lg:w-[400px] bg-white border-l border-gray-100 p-8 flex flex-col shadow-2xl">
        <div className="mb-8 space-y-4">
          <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100 relative group overflow-hidden">
            ...
            <button onClick={() => setIsCustomerModalOpen(true)} className="absolute top-6 right-6 text-gray-300 hover:text-green-700"><UserPlus size={22} /></button>
          </div>
          
          <div className="relative">
            <input type="text" placeholder="NIT..." value={nitInput} onChange={(e) => setNitInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && buscarCliente()} className="w-full pl-6 pr-14 py-4 bg-gray-50 border-2 border-transparent focus:border-green-700 rounded-2xl outline-none font-bold" />
            <button onClick={buscarCliente} className="absolute right-3 top-1/2 -translate-y-1/2 bg-green-700 text-white p-2.5 rounded-xl shadow-lg shadow-green-900/20"><Search size={20} /></button>
          </div>
          ...
          <div className="relative pt-4 border-t border-gray-100">
             ...
             <div className="relative">
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-orange-50/50 border-2 border-transparent focus:border-orange-500 rounded-2xl outline-none font-bold text-gray-700" 
                />
                <Keyboard className="absolute left-4 top-1/2 -translate-y-1/2 text-orange-400" size={20} />
             </div>
          </div>
        </div>
        ...
        <div className="flex-1 bg-gray-50/50 rounded-[2rem] p-6 border border-gray-100">
          <div className="space-y-4">
            <div className="flex justify-between text-gray-500 font-bold italic"><span>Subtotal</span><span>Q{(total / 1.12).toFixed(2)}</span></div>
            <div className="flex justify-between text-gray-500 font-bold italic"><span>IVA (12%)</span><span>Q{(total - (total / 1.12)).toFixed(2)}</span></div>
            <div className="border-t border-dashed pt-6 flex flex-col items-end">
        ...
        <div className="mt-8 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <button disabled={loading || cart.length === 0} onClick={() => handleCheckout('Efectivo')} className="flex flex-col items-center gap-3 p-6 bg-white border-2 border-gray-100 rounded-[2rem] hover:border-green-700 shadow-sm transition-all group">
              <div className="bg-green-50 p-3 rounded-2xl text-green-700 group-hover:bg-green-700 group-hover:text-white transition-all">{loading ? <Loader2 className="animate-spin" /> : <Banknote size={28} />}</div>
              <span className="text-xs font-black uppercase text-gray-700 tracking-widest">Efectivo</span>
            </button>
            <button disabled={loading || cart.length === 0} onClick={() => handleCheckout('Tarjeta')} className="flex flex-col items-center gap-3 p-6 bg-white border-2 border-gray-100 rounded-[2rem] hover:border-orange-500 shadow-sm transition-all group">
              <div className="bg-orange-50 p-3 rounded-2xl text-orange-500 group-hover:bg-orange-500 group-hover:text-white transition-all">{loading ? <Loader2 className="animate-spin" /> : <CreditCard size={28} />}</div>
              <span className="text-xs font-black uppercase text-gray-700 tracking-widest">Tarjeta</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {showScanner && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white p-8 rounded-[3rem] w-full max-w-md border-4 border-green-700">
            <h3 className="text-xl font-black mb-6 text-gray-800 italic uppercase flex items-center gap-3"><Receipt className="text-green-700" /> Escáner</h3>
            <div className="rounded-2xl overflow-hidden"><BarcodeScanner onScan={handleScan} /></div>
            <button onClick={() => setShowScanner(false)} className="w-full mt-6 text-gray-400 font-black uppercase tracking-widest py-4">Cerrar</button>
          </div>
        </div>
      )}
