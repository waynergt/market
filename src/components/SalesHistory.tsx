    ...
    <div className="p-4 md:p-8 space-y-6">
      <div>
        <h1 className="text-3xl font-black text-gray-800 flex items-center gap-3">
          <History className="text-blue-600" /> Historial de Ventas
        </h1>
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredSales.map((sale) => (
              <tr key={sale.id} className="hover:bg-blue-50/30 transition-colors">
                ...
                <td className="p-5">
                  <span className={`flex items-center gap-1 text-xs font-bold ${sale.payment_method === 'Efectivo' ? 'text-green-600' : 'text-blue-600'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b flex justify-between items-center bg-gray-800 text-white">
              ...
              <button onClick={() => setSelectedSale(null)} className="hover:bg-gray-700 p-2 rounded-full"><X /></button>
            </div>

            <div className="p-6 space-y-4">
              {loading ? (
                <div className="text-center p-10 text-gray-400 animate-pulse font-bold">Cargando productos...</div>
              ) : (
                <>
                  ...
                  <div className="pt-4 flex justify-between items-center">
                    ...
                    <span className="text-3xl font-black text-blue-600">Q{selectedSale.total.toFixed(2)}</span>
                  </div>
                </>
              )}
            </div>
